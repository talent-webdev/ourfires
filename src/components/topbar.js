...
console.log("is it working?");
  ...
  // THIS IS A MOCKUP APPROXIMATION OF WHAT WILL GET BACK FROM THE CMS
  const data = {
    site: {
      siteMetadata: {
        menuLinks: [
  ...
  return (
    ...
    >
      ...
      >
        <Stack
          className="navigationMenu" /* for hover state */
          fontSize={{ base: "titleBig", sm: "small" }}
          fontWeight="bold"
          ...
          pt={{ base: "16", sm: "0" }}
        >
          {data.site.siteMetadata.menuLinks.map((menuItem, key) => (
            <Link key={key} to={menuItem.link} activeStyle={{ color: theme.colors.primary }}>
